...
| `!previous` | `!prev !pr` | `The previous command skips the the previous song in the queue and immediately begins playing it, if there is not previous song in the queue (i.e. you are at the first song in the queue) previous replays the current song.` | `Previous requires that you are connected to a voice channel.` |
| `!skip` | `!sk !next` | `The skip command skips to the next song in the queue and immediately begins playing it (assuming there is a next song in the queue).` | `The skip command requires that you are connect to a voice channel and that you are not at the end of the queue (there is a next song).` |
| `!replay` | `!rep` | `The replay command replays the currently playing or paused song from the beginning, if you reach the end of the queue after playing the last song, using replay will restart the last song in the queue.` | `Replay requires that you are connected to a voice channel, and that there are songs in the queue.` |
